...
class InstallerLifecycleTests(unittest.TestCase):
    def test_dry_run_receipt_update_diff_and_conservative_uninstall(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            dry = run("--target", "generic", "--destination", str(destination), "--skill", "omarchy-app-design", "--dry-run")
            self.assertEqual(0, dry.returncode, dry.stdout + dry.stderr)
            self.assertFalse(destination.exists())
            self.assertFalse(any(destination.parent.glob("*.lock")))

            installed = run("--target", "generic", "--destination", str(destination), "--skill", "omarchy-app-design")
            ...
            self.assertEqual(["omarchy-app-design"], diff_payload["wouldChange"])
            self.assertEqual(["SKILL.md"], diff_payload["diff"]["omarchy-app-design"]["changed"])
            ...
            self.assertFalse((destination / "omarchy-app-design").exists())
    ...
    def test_source_and_destination_symlinks_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = run("--target", "generic", "--destination", str(link / "skills"), "--skill", "omarchy-app-design")
    ...
    def test_hardlinked_lock_is_rejected_without_clobbering_peer(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            lock = root / ".skills.build-omarchy-apps.lock"
            os.link(victim, lock)
            result = run("--target", "generic", "--destination", str(destination), "--skill", "omarchy-app-design")
            self.assertEqual(2, result.returncode)
    ...
    def test_multi_skill_install_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(10, len(json.loads(first.stdout)["installed"]))
            second = run("--target", "generic", "--destination", str(destination))
            self.assertEqual(0, second.returncode, second.stdout + second.stderr)
            self.assertEqual(10, len(json.loads(second.stdout)["unchanged"]))
