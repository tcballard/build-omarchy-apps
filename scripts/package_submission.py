...
SOURCE_SCHEMA = "https://github.com/tcballard/build-omarchy-apps/blob/main/schemas/source-manifest-v1.json"
RELEASE_SCHEMA = "https://github.com/tcballard/build-omarchy-apps/blob/main/schemas/release-manifest-v1.json"
...
def _source_manifest(version: str, commit: str, tree: str, blobs: dict[PurePosixPath, Blob]) -> bytes:
    return _json_bytes({
        "schemaVersion": 1,
        "name": "build-omarchy-apps",
        "version": version,
        "source": {
            "repository": "https://github.com/tcballard/build-omarchy-apps",
            "commit": commit,
            "tree": tree,
        },
        "files": [
...
def _sbom(version: str, commit: str, blobs: dict[PurePosixPath, Blob]) -> bytes:
    ...
    return _json_bytes({
        ...
        "name": f"build-omarchy-apps-{version}-source",
        "documentNamespace": f"https://github.com/tcballard/build-omarchy-apps/spdx/{version}/{commit}",
        "creationInfo": {"created": created, "creators": ["Tool: build-omarchy-apps/package_submission.py"]},
        "packages": [{
            "name": "build-omarchy-apps",
            "SPDXID": "SPDXRef-Package",
            "versionInfo": version,
            "downloadLocation": f"git+https://github.com/tcballard/build-omarchy-apps.git@{commit}",
            "filesAnalyzed": False,
...
def build(repo: Path, output: Path, revision: str, require_clean: bool) -> dict[str, Any]:
    ...
    try:
        ...
        adapter = _json_blob(blobs[PurePosixPath("plugins/build-omarchy-apps/.codex-plugin/plugin.json")])
    ...
    policy_names = {"VERSION", "README.md", "PORTABILITY.md", "LICENSE", "SECURITY.md", "PRIVACY.md", "TERMS.md", "SUPPORT.md"}
    portable_entries = _selected(
        blobs,
        lambda p: p == PurePosixPath("plugin.json") or p.parts[:1] == ("skills",) or p.as_posix() in policy_names or p.parts[:1] in {("scripts",), ("docs",), ("evals",)} or p.as_posix() == "NOTICE.md",
        lambda p: PurePosixPath("build-omarchy-apps") / p,
    )
    claude_entries = _selected(
        blobs,
        lambda p: p in {PurePosixPath(".claude-plugin/plugin.json"), PurePosixPath("plugin.json")} or p.parts[:1] == ("skills",) or p.as_posix() in policy_names or p.parts[:1] in {("scripts",), ("docs",), ("evals",)} or p.as_posix() == "NOTICE.md",
        lambda p: PurePosixPath("build-omarchy-apps") / p,
    )
    adapter_prefix = PurePosixPath("plugins/build-omarchy-apps")
    adapter_entries = _selected(
        blobs,
        lambda p: p.parts[:2] == ("plugins", "build-omarchy-apps"),
        lambda p: PurePosixPath("build-omarchy-apps") / p.relative_to(adapter_prefix),
    )
    skills_prefix = PurePosixPath("plugins/build-omarchy-apps/skills")
    skills_entries = _selected(
        blobs,
        lambda p: p.parts[:3] == ("plugins", "build-omarchy-apps", "skills"),
        lambda p: PurePosixPath("skills") / p.relative_to(skills_prefix),
    )
    source_entries = _selected(blobs, lambda p: True, lambda p: PurePosixPath("build-omarchy-apps") / p)
    archive_specs = (
        ("agent-plugin", f"build-omarchy-apps-agent-plugin-{version}.zip", portable_entries),
        ("claude-plugin", f"build-omarchy-apps-claude-plugin-{version}.zip", claude_entries),
        ("openai-plugin", f"build-omarchy-apps-plugin-{version}.zip", adapter_entries),
        ("openai-skills", f"build-omarchy-apps-skills-{version}.zip", skills_entries),
        ("source", f"build-omarchy-apps-source-{version}.zip", source_entries),
    )
    ...
    release = {
        ...
        "name": "build-omarchy-apps",
        "version": version,
        "source": {"repository": "https://github.com/tcballard/build-omarchy-apps", "commit": commit, "tree": tree},
        "artifacts": artifacts,
