...
| Prepare package | Package → Test → development handoff → Submit when requested |
...
Every development PR/archive gets the handoff checks in the selected skill. Missing live access remains visible without blocking a scoped development contribution. App publication, package contribution and production repository promotion are separate actions. Preserve already-given authorization; do not invent extra confirmation loops.
